...
def updateImpactsDict(args, fitresult, df, lumiscale, poi='Wmass'):
    ...
    if args.nuisance:
        ...
        'ZmassAndWidth' 'stat' 'binByBinStat' 'Total', 'recoil_stat'] '''

    else:
        plot_labels = ['Total', 'Theory', 'PDF', 'Experiment', 'Recoil (sub-experiment)', 'Data Stat.', 'Luminosity']
        plot_vals = [impacts[list(labels).index('Total')], impacts[list(labels).index('theory')], \
                    impacts[list(labels).index('pdfNNPDF31')], impacts[list(labels).index('experiment')], \
                    impacts[list(labels).index('recoil_stat')], impacts[list(labels).index('stat')], \
                    lumiVal]
                    # otherwise, PDFNN31
...
if __name__ == '__main__':
    ...
    if args.nuisance:
        ...
        df = pd.DataFrame(columns=['Total', 'Theory', 'PDF', 'Experiment', 'Recoil (sub-experiment)', 'Data Stat.', 'Luminosity'])
    ...
    colors = ['steelblue', 'crimson', 'darkorange', 'olivedrab', 'olivedrab', 'mediumpurple', 'lightpink']
    for i, column in enumerate(df.columns):
        if column != 'Luminosity':  # Exclude the Luminosity column from plotting
            if "sub" not in column:
                plt.plot(df['Luminosity'], df[column], label=column, marker='o', color = colors[i])
            else:
                plt.plot(df['Luminosity'], df[column], label=column, marker='o', linestyle = "--", color = colors[i])

    # plt.title("lnN: 1.02; nnpdf, without minnloScaleUnc/resumUnc.", y = 1.07, fontsize = 16)
    plt.title("Uncertainties, including recoil, over luminosity", y = 1.07, fontsize = 16)
